...
|---|---|---|---|
| "Right-side text/labels are cut off" | `layout.margin.r` small (or `annotations[].x ≥ 0.95` with `xref='paper'` and small `margin.r`) | `fig.update_layout(margin=dict(r=80))` | https://plotly.com/python/setting-graph-size/ |
| "Legend covers the data" | `layout.legend.x` < `1.0` (legend extends rightward from `x` into the plot) | `fig.update_layout(legend=dict(x=1.05, xanchor='left'))` | https://plotly.com/python/legend/ |
| "Text labels clipped at the axis edge" | trace `cliponaxis` absent (default `True`, text mode) — see audit `cliponaxis_text` / Depth 2 | `fig.update_traces(cliponaxis=False, selector=...)` | https://plotly.com/python/figure-introspection/ |
...
| "Color scale is wrong / not perceptually uniform" | `marker.colorscale` (or `layout.coloraxis.colorscale` for px color-axes) is qualitative on numeric, or reversed | set a sequential/diverging `colorscale` (e.g. `'Viridis'`), `reversescale=True` | https://plotly.com/python/colorscales/ |
