...
async def test_r_base_sidecar_loaded_at_start(monkeypatch, tmp_path):
    ...
    async with Client(mcp) as client:
        r = await client.call_tool("run_code", {"session": "r:rsc", "code": "who()"})
...
async def test_r_peek_summarizes_dataframe(monkeypatch, tmp_path):
    ...
    from repl_server import mcp
    async with Client(mcp) as client:
        await client.call_tool("run_code", {"session": "r:rpk", "code": "df <- data.frame(a=1:3, b=c('x','y','z'))"})
        r = await client.call_tool("run_code", {"session": "r:rpk", "code": "peek(df)"})
