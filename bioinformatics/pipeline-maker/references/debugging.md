...
3. **Executor-plugin (SLURM/HPC) job logs — for scheduler-level verdicts.** The slurm executor plugin's default per-job log is `.snakemake/slurm_logs/rule_<rulename>/<wildcard-values>/<jobid>.log` — the rule name is a directory, the job's wildcard values form the next segment, and the SLURM job id is the filename (e.g., `.snakemake/slurm_logs/rule_Module_3_PLINK_Convert_To_PGEN/chr1_130000001_135000000_0/6318491.log`). The location is configurable via the plugin's log setting in the profile — the WGS pipeline had customized it to `logs/slurm/rule_<rulename>/<jobid>.log` — so **read the profile to confirm**. When the rule's own log is empty or clean, the SLURM job log carries the scheduler verdict: `OUT_OF_MEMORY`, `TIME_LIMIT`, exit codes. Also check the rendered jobscript (the plugin writes it under `.snakemake/`) to see exactly what was submitted, and `sacct -j <jobid>` for the scheduler state.
