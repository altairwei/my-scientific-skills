...
Run: `uv run --with plotly --with pandas --with pytest python -m pytest data-science/plotly-interactive-figures/tests/ -v`
Expected: all PASS (37 tests: 6 entrypoint + 5 empty/dup + 6 cliponaxis/hover + 6 legend/colorway + 8 log/range + 6 margin).
...
**2g. Task 12 consolidation (review-minor fixes applied at the final pass):** (1) legend-bounds tests added (left-edge positive + exact-boundary negatives at `x=1.0`/`x=0.05` — pins the 2d bounded-zone fix against regression) + `xref='container'` guard in `_check_legend_off_canvas`; (2) SKILL.md step-3 triage now distinguishes data-loss warns (`axis_range_excludes_data` — treat like errors) from advisory ones (`cliponaxis_text`) and mentions `info` findings; (3) SKILL.md "When NOT to use" gains the FigureWidget/callback non-goal; (4) `figure-style`'s "When NOT to use" gains the reverse cross-link to this skill (per the design spec's relationship section); (5) design spec's `audit(fig_or_spec, df=None)` signature + "(needs `df`)" check-table rows updated to the shipped `audit(fig_or_spec)` (self-contained spec); (6) symptom-prescription.md checkpoints reworded (legend `x < 1.0` covers-data zone, `cliponaxis` absent-key phrasing, `layout.coloraxis.colorscale` px path, annotation `xref='paper'` gate); (7) notebook-delivery.md exit-code wording (hard errors exit 1; the silent text-repr degradation is the real hazard).
