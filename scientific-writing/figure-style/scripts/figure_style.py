"""figure_style — publication-grade matplotlib helpers (import-only module).

This is NOT a CLI. Import it from your plotting script and run that script
under ``uv run --with``:

    import sys
    sys.path.insert(0, "<plugin-root>/scientific-writing/figure-style/scripts")
    import figure_style as fs

    fs.apply_figure_style()                     # before plotting anything
    ...
    fig.savefig("figure.png")                   # 300 dpi, tight bbox via rcParams
    import json
    json.dump(fs.panel_crops(fig), open("crops.json", "w"), indent=1)

Run:

    uv run --with matplotlib --with numpy python plot.py

(`scipy` is needed only for bar_with_points(errorbar="ci95") — add
``--with scipy`` if you use it.)

The §-references in docstrings point at references/rules.md in this skill —
the checklist these helpers encode.
"""
...
def panel_crops(fig, dpi=None, pad_px=6, bbox_inches=None, pad_inches=None):
    ...
    top-left, matching ``PIL.Image.crop``). Dump to JSON after savefig, crop
    the saved PNG with any tool (e.g. the pdf-explore skill's
    ``pdf_explore.py crop PNG --box x0,y0,x1,y1`` or 3 lines of PIL), then
    Read each crop for the perceptual check. Panels are detected as bold
    single-character ``Text`` objects placed by :func:`panel_letter`; each
    panel's crop is its axes' tightbbox mapped into the saved file's pixel
    space, padded by ``pad_px``. For §3.4 composites (abutting subplots
    sharing an axis, letter on the leftmost only) the crop unions in
    letterless ``sharex``/``sharey`` siblings on the same grid row/col so the
    whole composite is covered. When no axes carries a panel letter
    (standalone plot, or a figure-composer panel agent), falls back to one
    crop per axes keyed by index.
    ...
    *consult rcParams* (so under :func:`apply_figure_style` it resolves to
    ...
    boxes are clamped to the saved image extent regardless.
        ...
        >>> import json; json.dump(panel_crops(fig), open("crops.json", "w"))
    """
    import matplotlib as mpl
    ...
    for ax in fig.axes:
        for t in ax.findobj(matplotlib.text.Text):
            ...
            if len(s) == 1 and s.isalpha() and t.get_fontweight() in ("bold", 700):
                lettered[ax] = s
                break
    # No panel letters (standalone plot, or a figure-composer panel agent which
    # is told NOT to draw its own letter): fall back to one crop per axes so
    # the §9.2 loop still inspects something instead of silently iterating
    # over {}.
    if not lettered:
